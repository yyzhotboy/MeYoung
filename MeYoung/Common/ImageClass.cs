...
namespace Common
{
    ...
    public class ImageClass
    {
        ...
        /// 生成缩略图
        /// </summary>
        /// <param name="int_Width">高度</param>
        /// <param name="int_Height">宽度</param>
        /// <param name="imgFullPath">图片地址</param>
        /// <param name="thumbnailType">生成图片的类型</param>
        public static void MakeThumbnail(int int_Width, int int_Height, string imgFullPath, ThumbnailType thumbnailType)
        {

            System.Drawing.Image oldimage = System.Drawing.Image.FromFile(imgFullPath);
            ...
            string filePath = imgFullPath.Substring(0, (imgFullPath.Length - fileName.Length));

            string fileSuffix = "_"+thumbnailType.ToString()+".";
            ...
            if (flat != 0)
            {
                ...
                try
                {
                    bmOutput.Save(filePath + fileName.Split('.').GetValue(0).ToString() + fileSuffix + fileName.Split('.').GetValue(1).ToString());
                }
            ...
            else
            {
                ...
                try
                {
                    bm.Save(filePath + fileName.Split('.').GetValue(0).ToString() + fileSuffix + fileName.Split('.').GetValue(1).ToString());
                }
                catch (System.Exception e)
                {
                    throw e;
        ...
        /// 获取缩略图地址
        /// </summary>
        /// <param name="imgPath">数据库中保存的图片地址</param>
        /// <param name="thumbnailType">要获取的缩略图类型</param>
        /// <returns>缩略图地址</returns>
        public static string GetThumbnail(string imgPath, ThumbnailType thumbnailType)
        {
            string thumbailPath = string.Empty;
            if (thumbnailType == ThumbnailType.None)
            {
                thumbailPath = imgPath;
            }
            else
            {
                string fileName = imgPath.Substring(imgPath.LastIndexOf("/") + 1);
                string filePath = imgPath.Substring(0, (imgPath.Length - fileName.Length));

                string fileSuffix = "_" + thumbnailType.ToString() + ".";
                thumbailPath = filePath + fileName.Split('.').GetValue(0).ToString() + fileSuffix + fileName.Split('.').GetValue(1).ToString();
            }
            return thumbailPath;
        }
